...
/**
 * The Spring controller for the {@link DatabaseExService}, whose timed
...
public class DatabaseExControllerTimed {
    /**
     * Spring injected {@link DatabaseExController}.
     */
    @Autowired
    private DatabaseExController mController;
     ...
     *
     * @return A {@link Flux} that emits all movies in the database
     */
    @GetMapping(GET_ALL_MOVIES)
    public Flux<Movie> getMoviesTimed() {
        return mRunTimer
            // Forward request.
     ...
     * @return A {@link Flux} that emits movie titles containing
     *         the query represented as {@link Movie} objects
     */
    @GetMapping(GET_SEARCH + "/" + SEARCH_QUERY)
    public Flux<Movie> searchTimed(@PathVariable String query) {
        return mRunTimer
            // Forward request.
            .runAndRecordTime(mController.getId()
                              + ":"
     ...
     * @return A {@link Flux} that emits movie titles containing any
     *         queries represented as {@link Movie} objects
     */
    @PostMapping(POST_SEARCHES)
    public Flux<Movie> searchTimed(@RequestBody List<String> queries) {
        return mRunTimer
            // Forward request.
            .runAndRecordTime(mController.getId()
                              + ":"
     ...
     * @param queries The search queries
     * @return A {@link Flux} that emits movie titles containing
     *         all queries represented as {@link Movie} objects
     */
    @PostMapping(POST_SEARCHES_EX)
    public Flux<Movie> searchTimedEx(@RequestBody List<String> queries) {
        return mRunTimer
            // Forward request.
            .runAndRecordTime(mController.getId()
                              + ":"
