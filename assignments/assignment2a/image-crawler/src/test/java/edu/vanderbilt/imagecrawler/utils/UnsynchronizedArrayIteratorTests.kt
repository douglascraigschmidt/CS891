 ...
 */
open class UnsynchronizedArrayIteratorTests : AssignmentTests() {
    /**
    ...
    @Test
    fun `iterator must handle an empty array`() {
        if (skipTest) return
    ...
    @Test
    fun `iterator hasNext() and next() should work in conjunction to properly allow access to all array elements`() {
        if (skipTest) return
    ...
    @Test
    fun `iterator next() must throw NoSuchElementException when array is empty`() {
        if (skipTest) return
    ...
    @Test
    fun `iterator next() must throw NoSuchElementException for all calls after end of array is reached`() {
        if (skipTest) return
    ...
    @Test
    fun `iterator add() and set() methods must work as expected`() {
        if (skipTest) return
    ...
    @Test
    fun `iterator remove() must be able to remove all elements`() {
        if (skipTest) return
