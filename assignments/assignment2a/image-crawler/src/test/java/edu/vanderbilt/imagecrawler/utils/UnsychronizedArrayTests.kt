 ...
 */
class UnsychronizedArrayTests : AssignmentTests() {
    /**
    ...
    fun `ensureInternalCapacity() must allocate memory efficiently`() {
        ...
        assertTrue(allocs.toString() + " memory allocations occurred for parameters [1..1000]; " +
                "allocation algorithm needs to be more efficient!",
                allocs < 14)

        assertTrue(allocs.toString() + " memory allocation(s) occurred for parameters [1..1000]; " +
                "allocation algorithm needs to be more efficient!",
                allocs > 1)
    }
