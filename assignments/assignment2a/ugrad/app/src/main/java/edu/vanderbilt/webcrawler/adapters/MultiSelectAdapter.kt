...
abstract class MultiSelectAdapter<T, VH : SelectableViewHolder>(
        ...
        private val onSelectionListener: OnSelectionListener? = null) : RecyclerView.Adapter<VH>() {
    ...
    val selectedItemPositions: List<Int>
        get() {
            return (wrappedItems.indices).filter { wrappedItems[it].selected }.toList()
        }
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        with (holder) {
            ...
            itemView.setOnLongClickListener {
                if (actionMode == null) {
                    if (onSelectionListener?.onActionModeStarting() != false) {
            ...
            return startSupportActionMode(
                object : ActionMode.Callback {
                    ...
                    override fun onActionItemClicked(mode: ActionMode, item: MenuItem): Boolean {
                        when (item.itemId) {
                            R.id.action_delete -> {
                                // TODO: this needs to be changed if app needs to be
                                // responsible for deleting adapter items.
                            ...
                            R.id.action_select_all -> {
                                ...
                                if (onSelectionListener != null) {
                                    if (!onSelectionListener.onActionModeCommand(
    ...
    fun finishActionMode() {
        ...
        actionMode?.finish()
        actionMode = null
        onSelectionListener?.onActionModeFinished()
    }
    ...
    fun toggleSelection(position: Int): Boolean {
        wrappedItems[position].selected = !wrappedItems[position].selected
        notifyItemChanged(position)
    ...
    fun selectItem(position: Int, select: Boolean = true) {
        if (position in 0..wrappedItems.lastIndex) {
            with(wrappedItems[position]) {
                if (selected != select) {
                    selected = select
                }
                notifyItemChanged(position)
            ...
            // This algorithm uses the minimum number of notify calls.
            var changed: Int = 0
            wrappedItems.forEachIndexed { i, item ->
                if (item.selected != select) {
                    item.selected = select
                    changed++
                } else if (changed > 0) {
                    notifyItemRangeChanged(i - changed, changed)
                    changed = 0
                }
            }

            if (changed > 0) {
                notifyItemRangeChanged(wrappedItems.lastIndex - changed, changed)
            }
        } else {
    ...
    /**
     * Returns extra data object that is currently stored with the item at [position].
     */
    ...
    fun saveSelectionStates(): Bundle {
        val bundle = Bundle()
        bundle.putIntegerArrayList(KEY_SELECTION_POSITIONS, selectedItemPositions as ArrayList<Int>)
        return bundle
    }
    ...
    fun restoreSelectionStates(savedStates: Bundle) {
        val selectedPositions = savedStates.getIntegerArrayList(KEY_SELECTION_POSITIONS)
        selectItems(selectedPositions)
    }
    ...
    interface OnSelectionListener {
         ...
         * allow action mode to start, `false` to prevent action mode from
         * starting.
         */
        fun onActionModeStarting(): Boolean

        /**
         * Called when ActionMode has started.
         */
         ...
         * Delete and selecte all menu items are handled by this adapter
         ...
         */
        fun onActionModeCommand(selections: List<Int>, menuActionId: Int): Boolean
         ...
         * Return `true` to start action mode and `false` to prevent action
         * mode from starting.
         */
        fun onItemLongClick(view: View, position: Int): Boolean

        /**
         * Hook method called when any items are waiting for data. When [show] is true
