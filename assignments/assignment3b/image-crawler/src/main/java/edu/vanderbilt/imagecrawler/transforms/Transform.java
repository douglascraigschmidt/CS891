...
public abstract class Transform {
	...
	public enum Type {
		...
		NULL_TRANSFORM(NullTransform.class);

		private final Class<? extends Transform> clazz;
     ...
     * Debug logging tag.
     */
    protected final String TAG = Transform.class.getName();
    ...
    public Image transform(Image image) {
        ...
        Image result = applyTransform(image);
	...
	public static final class Factory {
		...
		private static Map<String, Class<? extends Transform>> map = new HashMap<>();
		...
		public void register(String name, Class<? extends Transform> clazz) {
			Class<? extends Transform> existingClazz = map.get(name);
			...
			map.put(name != null ? name : clazz.getClass().getSimpleName(), clazz);
		}
		...
		public static Transform newTransform(String name) {
			return newTransform(name, map.get(name));
		}
		...
		public static List<Transform> newTransformsFromNames(List<String> names) {
			...
			return names.stream()
					.map(name -> newTransform(name, map.get(name)))
		 ...
		 * @param name The transform class to create.
		...
		public static Transform newTransform(Type type, String name) {
			try {
				// Construct a new transform instance.
				Transform transform = type.clazz.newInstance();
