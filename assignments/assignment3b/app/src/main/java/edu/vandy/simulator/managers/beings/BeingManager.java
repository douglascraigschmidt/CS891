...
import edu.vandy.simulator.managers.beings.completionService.ExecutorCompletionServiceMgr;
...
public abstract class BeingManager<T extends Being>
     ...
     */
    @NotNull
    private List<T> mBeings = new ArrayList<>();
     ...
     */
    final void buildModel(Simulator simulator, int beingCount, int gazingIterations) {
    ...
    final public static class Factory {
        ...
        public static BeingManager newManager(Type type,
                                              ...
                                              Simulator simulator) {
            try {
                BeingManager manager = type.clazz.newInstance();
                manager.buildModel(simulator, beingCount, gazingIterations);
                return manager;
            } catch (Exception e) {
                e.printStackTrace();
        ...
        /**
         * Supported BeingManagers. The enum values are set to class
         * types so that they can be used to create BeingManager
         * objects using newInstance().
         */
        public enum Type {
            COMPLETION_SERVICE(ExecutorCompletionServiceMgr.class),
            NO_MANAGER(NoManager.class); // Do not remove or replace.

            public final Class<? extends BeingManager> clazz;

            Type(Class<? extends BeingManager> clazz) {
                this.clazz = clazz;
            }
        ...
        /**
         * An empty BeingManager implementation that can be used to
         * disable individual Type enum entries so that the will not
         * be included in the app's selectable list of BeingMangers.
         */
        public static class NoManager extends BeingManager<NoManager.NoBeing> {
            /**
             * Called to create and return a new concrete Being instance.
             *
             * @return A new Being concrete instance.
            ...
            public static class NoBeing extends Being {
                public NoBeing(BeingManager manager) {
                    super(manager);
                }
                @Override
