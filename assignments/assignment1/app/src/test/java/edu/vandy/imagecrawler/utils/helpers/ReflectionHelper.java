package edu.vandy.imagecrawler.utils.helpers;
...
public class ReflectionHelper {
    ...
    public static <T> T findFirstMatchingFieldValue(Object parentClass, Class findClass, String name) {
        Field field = findFirstMatchingField(parentClass, findClass, name);
        if (field == null) {
            return null;
        } else {
            try {
    ...
    public static Field findFirstMatchingField(Object parentClass, Class findClass, String name) {
        Field[] declaredFields = parentClass.getClass().getDeclaredFields();
        return Arrays.stream(declaredFields)
                .filter(field -> {
                    if (field.getType() != findClass) {
                        return false;
    ...
    public static void injectValueIntoMatchingField(
        ...
        Field field = findFirstMatchingField(parentClass, fieldClass, name);
        if (field == null) {
            throw new IllegalAccessException(
                    "Unable to inject field value into class field of type "
                            + fieldClass.getName());
