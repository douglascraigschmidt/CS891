 ...
 */
open class KCache(val cacheDir: File) {
    companion object {
        val NOTAG = "__notag__"
    }
    ...
    private val allStates = Operation.values()

    /** Map for handling concurrent access. */
    private val hashMap = ConcurrentHashMap<String, Item>()
     ...
     */
    private data class ObserverEntry(val filter: Array<out Operation>,
     ...
     */
    interface Observer {
        fun event(operation: Operation, item: Item, progress: Float = -1f)
    }
     ...
     */
    private fun newItem(key: String, tag: String): Item
     ...
     */
    fun startWatching(observer: Observer, vararg operations: Operation = allStates) {
     ...
     * Gets a cache item.
     */
    fun get(key: String, tag: String?): Item? {
     ...
     */
    fun put(key: String, tag: String?): Item? {
     ...
     * item is returned.
     */
    fun putIfAbsent(key: String, tag: String?): Item? {
        if (key.startsWith(NOTAG)) {
            error("Invalid argument: key cannot begin with $NOTAG")
        }
        ...
        if (oldItem == null) {
            ...
            notifyObservers(item, Operation.CREATE)
        }
    ...
    fun contains(key: String, tag: String?): Boolean {
        return hashMap.contains(getKey(key, tag ?: NOTAG))
    }

    fun remove(key: String, tag: String?): Item? {
        val item = hashMap.remove(getKey(key, tag ?: NOTAG))
        item?.let {
            it.file.delete()
            notifyObservers(it, Operation.DELETE)
        }
     ...
     */
    fun newItemFromFile(file: File): Item {
        if (!file.exists()) {
     ...
     */
    private fun notifyObservers(item: Item,
                                operation: Operation,
                                progress: Float = -1f) {
    ...
    private fun notifyCurrentContents(observers: List<ObserverEntry>) {
        hashMap.forEach { item ->
            observers.filter { it.filter.contains(Operation.LOAD) }
                    .forEach { it.observer.get()?.event(Operation.LOAD, item.value) }
        }
    }
    ...
    data class Item(val file: File,
                    val key: String,
                    val tag: String = NOTAG,
                    val timestamp: Long,
                    private val notify: (item: Item,
                                         operation: Operation,
                                         progress: Float) -> Unit) {
        ...
        fun getInputStream(operation: Operation): InputStream? {
            return try {
        ...
        fun getOutputStream(operation: Operation,
    ...
    class ObserverInputStream(inputStream: InputStream,
        ...
        private fun notify(operation: Operation, progress: Float) {
    ...
    class ObserverOutputStream(outputStream: OutputStream,
        ...
        override fun write(b: ByteArray?, off: Int, len: Int) {
            ...
            // progress will complete at roughly 100%.
            notify(operation, bytesWritten.toFloat() / (size / 3))
        }
