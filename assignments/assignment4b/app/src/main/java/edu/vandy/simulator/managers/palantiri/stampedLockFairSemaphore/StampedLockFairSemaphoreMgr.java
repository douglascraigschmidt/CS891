 ...
 * number of available Palantiri.  This class uses a FairSemaphore,
...
public class StampedLockFairSemaphoreMgr
    ...
    public void reset() {
        ...
        // to create a new FairSemaphore for the next run.
    ...
    /**
     * Creates and returns a new fair semaphore.
     *
     * @return A new FairSemaphore implementation.
     */
    protected FairSemaphore buildFairSemaphore() {
        if (Assignment.isUndergraduateTodo()) {
            return new FairSemaphoreMO(mPalantiriMap.size());
        } else if (Assignment.isGraduateTodo()) {
            return new FairSemaphoreCO(mPalantiriMap.size());
        } else {
            throw new IllegalStateException("Assignment type is not valid");
        }
    }
