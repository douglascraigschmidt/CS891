...
public class Assignment_2B_SimpleSemaphoreTest extends AssignmentTests {
    ...
    @Test
    public void testAcquireMultipleAwaitCalls() throws InterruptedException {
        if (Helpers.isReentrantLockSolution(mSimpleSemaphore)) {
            System.out.println("ReentrantLock solution ignored by Lock solution tests.");
            return;
        }
        ...
        verify(mLockMock).lock();
        verify(mNotZeroMock, times(PALANTIRI_COUNT + 1)).await();
        verify(mLockMock).unlock();

        inOrder.verify(mLockMock).lock();
        inOrder.verify(mNotZeroMock, times(PALANTIRI_COUNT + 1)).await();
        inOrder.verify(mLockMock).unlock();
