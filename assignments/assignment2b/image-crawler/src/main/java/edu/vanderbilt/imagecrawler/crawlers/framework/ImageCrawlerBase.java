...
import edu.vanderbilt.imagecrawler.crawlers.ParallelStreamsCrawler;
...
import edu.vanderbilt.imagecrawler.transforms.CachingTransformDecorator;
...
public abstract class ImageCrawlerBase
    ...
    void initialize(Controller controller) {
        ...
        // Store the transformations to applyTransform to each downloaded image.
     ...
     */
    private TransformDecoratorWithImage makeFilterDecoratorWithImage
        (Transform transform,
         Image image) {
        // Create a new cache entry to pass to the
        // CachingTransformerDecorator.
        ImageCache.Item item =
            mImageCache.newItem(transform.getName(),
                                image.getSourceUrl().toString());

        return new TransformDecoratorWithImage
            (new CachingTransformDecorator(transform,
                                           item),
             image);
    }
     ...
     */
    protected Image applyTransform(Transform transform, Image image) {
        return makeFilterDecoratorWithImage(transform, image).run();
    }

    /**
     * Checks to see if the {@code image} transformation {@code transform}
     * already exists in the cache. If not, the cache isItemCached()
     * method will atomically creates a new file based on
     * combining the image url with the transform name.
     *
    ...
    protected boolean imageCached(Image image,
        ...
        boolean isCached =
                mImageCache.isItemCached(groupId,
                                         image.getSourceUrl().toString());
     ...
     * combining the {@code url} with the {@code name} of the transform and
    ...
    protected Image getImage(URL url) {
        // If the image isn't cached, the download it and return the result.
        // Note that this isUrlCached call will automatically create a new
        // cache item if the image was not cached.
        if (!isUrlCached(url, null)) {
            return downloadImage(url);
        } else {
            // The image is not cached, so get instance of the new cache item
            // created by the isUrlCached call above.
            ImageCache.Item item =
   ...
   /**
    ...
    /**
     * Supported transforms that can be applied to downloaded
     * images. The enum values are set to class types so that
     * they can be used to create Transform objects using
     * newInstance().
     */
    public enum Type {
        SEQUENTIAL_LOOPS(SequentialLoopsCrawler.class),
        PARALLEL_STREAMS(ParallelStreamsCrawler.class);
