...
//    }
    ...
    private fun handleCacheEvent(items: List<Resource>) {
        if (viewModel.crawlCancelled || crawlProgress.state == CANCELLED) {
            warn {
    ...
    private fun handleProgressEvent(progress: MainViewModel.CrawlProgress) {
        // Ignore all RUNNING events sent after a CANCELLED event.
        if (crawlProgress.state == CANCELLED && progress.state == RUNNING) {
            return
        }
    ...
    internal fun updateTitle() {
        if (elapsedTime == -1L) {
            ...
            strategyValue.text = crawlStrategy.toString()
        }
    }
...
//            })
    ...
    private fun updateViewStates() {
        //info("updateViewStates: ${crawlProgress.state}")
