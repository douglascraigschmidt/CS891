...
object AndroidPlatform : Platform, KtLogger {
    ...
    /** Platform dependant cache implementation. */
    @JvmField
    val cache: AndroidImageCache = AndroidImageCache
     ...
     */
    override fun log(msg: String, vararg args: Any?)
            = debug("DIAGNOSTICS: ${if (args.isEmpty()) msg else msg.format(*args)}")
    ...
    override fun mapUriToInputStream(uri: String): InputStream? {
        ...
        val item = cache.get(uri, null)

        // Get the input stream.
        return when {
            uri.startsWith(assetsPrefix) -> {
                val inputStream = App.instance.assets.open(
                        Platform.LOCAL_WEB_PAGES_DIR_NAME
                        + "/" +
                        uri.removePrefix(assetsPrefix))
                ...
                if (item == null) {
                    ...
                    FileCache.ObserverInputStream(
                            inputStream,
                            Cache.Operation.LOAD,
                            item,
